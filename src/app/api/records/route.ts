...
export async function GET() {
  ...
  const sql = getSql();
  const records = await sql`SELECT * FROM archive_records WHERE published = true ORDER BY updated_at DESC`;
...
export async function POST(request: Request) {
  ...
  const rows = await sql`
    INSERT INTO archive_records (slug, type, title, summary, description, status, role, need, contradiction, tags, connections, image_url, published)
    VALUES (${value.slug}, ${value.type}, ${value.title}, ${value.summary}, ${value.description}, ${value.status}, ${value.role}, ${value.need}, ${value.contradiction}, ${JSON.stringify(value.tags)}::jsonb, ${JSON.stringify(value.connections)}::jsonb, ${value.imageUrl ?? null}, ${value.published})
    ON CONFLICT (slug) DO UPDATE SET type = EXCLUDED.type, title = EXCLUDED.title, summary = EXCLUDED.summary, description = EXCLUDED.description, status = EXCLUDED.status, role = EXCLUDED.role, need = EXCLUDED.need, contradiction = EXCLUDED.contradiction, tags = EXCLUDED.tags, connections = EXCLUDED.connections, image_url = EXCLUDED.image_url, published = EXCLUDED.published, updated_at = NOW()
    RETURNING *
  `;
